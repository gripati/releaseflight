import { NextResponse, type NextRequest } from "next/server";
import { z } from "zod";
import { transferLicense } from "@marquee/license";
import { requireRole, requireTenant } from "@/lib/auth";
import { assertCsrf, CSRF_HEADER } from "@/lib/csrf";
import { withApiErrors } from "@/lib/responses";

/**
 * Self-serve "move my license to this computer" — when the seat is held by an
 * old/decommissioned machine. Owner-only; rate-limited server-side.
 */
const Body = z.object({
  licenseKey: z.string().min(4).max(128),
  email: z.string().email().max(320),
  deviceLabel: z.string().max(120).optional(),
});

export const POST = withApiErrors(async (req: NextRequest) => {
  await assertCsrf(req.headers.get(CSRF_HEADER));
  const ctx = await requireTenant();
  requireRole(ctx.role, "OWNER");

  const body = Body.parse(await req.json());
  const result = await transferLicense({
    licenseKey: body.licenseKey,
    email: body.email,
    deviceLabel: body.deviceLabel,
  });
  return NextResponse.json(result);
});

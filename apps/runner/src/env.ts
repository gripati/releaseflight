/**
 * Loads the monorepo-root `.env` into `process.env` BEFORE any other runner
 * module evaluates.
 *
 * The runner is a standalone Node process (not Next.js), so nothing else loads
 * `.env` for it. Without this it starts with no `SECRETS_ENCRYPTION_KEY` /
 * `SECRETS_DIR` / `DATABASE_URL` / `REDIS_URL`, and the very first credential
 * it needs at deploy time fails with:
 *   "SECRETS_ENCRYPTION_KEY is required to read this encrypted secret."
 * (the Firebase / Apple / Play service-account is AES-GCM encrypted at rest by
 * the web app, which DOES load .env — the runner must share the same key).
 *
